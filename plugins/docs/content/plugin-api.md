...
| `{"template": "x.html", "data": {…}}` | Renders `x.html` from the active theme with goblog's usual page data plus your `data` merged in (your keys win). Two keys reach the page's `<head>`: `title` (the browser tab and Open Graph title) and `meta_description` (its `<meta name="description">` and Open Graph description, plain text). Set them on a page you want found — a page without its own description gets the site's. |
