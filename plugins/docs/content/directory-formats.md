...
4. On each goblog that should install from it, set `plugin_directory_url` to `https://<your site>/plugins/index.json` and `theme_directory_url` to `https://<your site>/themes/index.json` under **Admin → Settings → Directories**. The installer notices the changed URL on the next visit to **Admin → Plugins** or **Admin → Themes** and fetches the new index at once. To go back, set them to `https://www.goblog.live/plugins/index.json` and `https://www.goblog.live/themes/index.json` again.
